...
export function useUpdateCategory({category, expenses, period} : UpdateCategoryProps) {
    ...
    useEffect(() => {
        if (!userState?.user?.id || !category?.id || !period?.id) return;

        const fetchData = async () => {
            const res = await getResultByCategoryAndPeriod(userState.user.id, category.id, period.id)
            setResult(res)
        }
    ...
    async function updateResult (budget : Budget) {
        const result = await getResultByCategoryAndPeriod(userState.user.id, category.id, period.id)
